...
backtracking with an extra pruning step. There is also a time command that runs all the methods and displays
...
public class PA11Main {
    public static void main(String[] args) {
        ...
        if (args[1].equals("HEURISTIC")) {
            Trip bestTrip = heuristic(graph);
            System.out.println(bestTrip.toString(graph));
        }
        if (args[1].equals("BACKTRACK")) {
            Trip bestTrip = recursive(graph);
            System.out.println(bestTrip.toString(graph));
        }
        if (args[1].equals("MINE")) {
            Trip bestTrip = mine(graph);
            System.out.println(bestTrip.toString(graph));
        }
    ...
    public static DGraph createGraph(String[] args) {
        ...
        DGraph graph = new DGraph(nonZero);

        for (int i = 1; i < list.size(); i++) {
            graph.addEdge(Integer.valueOf(list.get(i)[0]),
